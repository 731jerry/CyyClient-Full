...
var DrawEverLine=function(lids,lwth,lpw){
    ...
    if($.browser.msie)
    {
        ...
        if(version>=19)
        {line = new FFLine("#F70000",1);}
        else
        {line = new IELine("#F70000",1);}
    }
    else
    {line = new FFLine("#F70000",1);}
    var tp = lids.split(",");
...
var DrawLine_blue = function(coords,tw,plw){
    var crd = coords.split("|");
    var tpwidth = tw.split("|");
    var tppl = plw.split("|");
    var len = crd.length;
    for(var i=0;i<len;i++)
    {
        ...
        {
            if(i==len-1)
            {DrawEverLine_blue(tp,lwidth,lpw);}
            else
            { window.setTimeout("DrawEverLine_blue('"+tp+"',"+lwidth+","+lpw+")",i*40);}
        }
        else
        {DrawEverLine_blue(tp,lwidth,lpw);}
    }
...
var DrawEverLine_blue=function(lids,lwth,lpw){
    var line;
    var wth = lwth;
    var pw = lpw;
    if($.browser.msie)
    {
        var version = parseFloat($.browser.version);
        if(version>=19)
        {line = new FFLine("#007DC0",1);}
        else
        {line = new IELine("#007DC0",1);}
    }
    else
    {line = new FFLine("#007DC0",1);}
    var tp = lids.split(",");
